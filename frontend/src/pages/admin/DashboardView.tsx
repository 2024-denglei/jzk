...
export function DashboardView() {
  ...
  const cards = [
    ...
    { label: '捐精人档案', value: donors, note: '当前档案总量', color: 'bg-violet-50 text-violet-600', icon: 'ri-archive-line' },
  ]
  return <div><PageHeader title="工作台" description="查看平台用户与捐精人档案的关键运营数据。" />{error ? <ErrorNotice message={error} /> : null}<div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{cards.map((card) => <div key={card.label} className="rounded-xl border border-[#dce4ee] bg-white p-5"><div className="flex items-start justify-between"><div><div className="text-xs text-[#738096]">{card.label}</div><div className="mt-2 text-3xl font-semibold">{card.value}</div><div className="mt-2 text-[11px] text-[#8e99a9]">{card.note}</div></div><div className={`flex h-11 w-11 items-center justify-center rounded-xl ${card.color}`}><i className={`${card.icon} text-xl`} /></div></div></div>)}</div><div className="mt-4 grid gap-4 lg:grid-cols-2"><QuickLink to="/admin/users" icon="ri-user-search-line" title="管理用户档案" description="查看账号状态、收藏、浏览历史和 AI 会话。" /><QuickLink to="/admin/donors" icon="ri-archive-drawer-line" title="维护捐精人档案" description="检索、编辑、启停捐精人及查看库存。" /></div></div>
}
