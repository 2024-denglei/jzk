...
def run_match_donors(session, feature_encoder, donor_df) -> tuple[list[dict], str, dict]:
    ...
    from jzk.domain.screening import compute_similarity, match_with_relaxation, diagnose_no_match
    from jzk.domain.screening_rank import rank_and_explain
    from jzk.advisor.dialogue_flow import build_feature_summary
    from jzk.advisor.session import DialogueState
...
def build_profile_summary(profile) -> str:
    from jzk.domain.preference.schema import EnumAttr, KeywordAttr, RangeAttr
