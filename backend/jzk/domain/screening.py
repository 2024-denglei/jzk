"""筛选：基于余弦相似度 + 欧氏距离，给用户自助查询用。不是匹配打分。"""

import numpy as np
from sklearn.metrics.pairwise import cosine_similarity, euclidean_distances

from jzk.config import COSINE_WEIGHT, EUCLIDEAN_WEIGHT, MATCH_THRESHOLD
...
def match_with_relaxation(
    ...
    """
    from jzk.config import MATCH_TOP_K as _CFG_TOP_K
...
def diagnose_no_match(
    ...
    返回: 瓶颈字段列表（中文标签）
    """
    from jzk.domain.screening_rank import _FIELD_LABEL  # 避免循环导入
    if threshold is None:
        threshold = MATCH_THRESHOLD
