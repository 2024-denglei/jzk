    ...
    <div className={adminPageShellClass('gap-0')}>
      <div className="shrink-0">
        ...
        <section className="rounded-xl border border-[#dce4ee] bg-white p-5">
          ...
          <div className="mt-5 grid gap-3 border-t border-[#e5eaf1] pt-4 sm:grid-cols-2 xl:grid-cols-6">
            ...
            <Info label="管理员操作" value={String(profile.admin_operation_count)} />
      ...
      <StickyTableCard
        ...
        toolbar={(
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#e2e8f0] px-5 py-3">
            ...
            <select value={source} onChange={(event) => { setSource(event.target.value); setPage(1) }} className="h-9 rounded-lg border border-[#d9e1ec] bg-white px-3 text-xs"><option value="">全部业务</option><option value="donor">捐精人档案</option><option value="user">用户管理</option><option value="admin">管理员管理</option></select>
          </div>
        )}
