...
允许独立收起。管理端只读取所选线路最后一轮 generation 的 Trace：该 Trace 的 `input_context`
就是最后一次实际提交给模型的完整线路上下文，工具调用、工具结果和最终回复继续接在同一时间线；
不再逐轮重复展示 System 和共享历史消息。完整排名只按这条最终 AI 消息懒加载。每次敏感读取写入
