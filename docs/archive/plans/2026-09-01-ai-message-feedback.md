...
| 阶段 | 状态 | 结果 |
|---|---|---|
| 数据库与契约 | 已完成 | 独立反馈表、消息/分支/用户强关联及迁移已落地 |
| 用户 API 与加载 | 已完成 | 支持选择、切换、取消，并随消息路径批量加载 |
| 客户端交互 | 已完成 | 已完成 AI 回复下展示喜欢/不喜欢图标并乐观更新 |
| 管理端反馈 | 已完成 | 全局列表、统计、筛选、游标分页和审计已落地 |
| 精确深链 | 已完成 | 可定位 Session、分支、锚点上下文并高亮具体回复 |
| 验证与上线 | 已完成 | 数据库迁移、自动化回归、构建和服务重启均纳入交付 |
...
新增迁移 `db/postgres/19_add_chat_message_feedback.sql`，并将它加入 `ensure_schema` 的固定迁移序列；全新环境在基础表和分支消息表建立后执行该迁移。
